function Move-VMOffline {
    <#
    .SYNOPSIS
        Moves a VM between hosts not joined in a domain.
    
    .DESCRIPTION
        Uses Hyper-V replica to move a VM between hosts that are not in a domain.
    ...
    .EXAMPLE
        Move-VMOffline -VMName "MyVM" -DestinationHost "Host2" -CertificateThumbprint "ABC123..."
        Moves MyVM to Host2 using certificate authentication.
    #>
    [CmdletBinding()]
    param(
