function Get-DebianImage {
    ...
    .DESCRIPTION
        Downloads the latest Debian cloud image and verifies its integrity.
    
    .PARAMETER OutputPath
        The directory to download the file to. If not specified, uses the current directory.
    
    .PARAMETER Previous
        If specified, downloads the previous version instead of the current one.
    ...
    .EXAMPLE
        $imgFile = Get-DebianImage -OutputPath $env:TEMP
        Downloads the latest Debian cloud image.
    #>
    [CmdletBinding()]
    param(
        [string]$OutputPath,
